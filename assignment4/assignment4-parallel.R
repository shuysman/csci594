...
### Calculate average 2020 historical annual precip for the Greater Yellowstone Ecosystem
...
rain <- rast(rain_files) %>%
    subset(year(time(.)) == 2020) %>% ### filter to years of interest
    clamp(lower = 0) ### Clamp off weird negative pixels over bodies of water, etc.

mean_rain <- terra::app(rain,

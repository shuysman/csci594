...
### Calculate average 2015-2020 historical annual precip for the Greater Yellowstone Ecosystem
...
rain <- rast(rain_files) %>%
    subset(year(time(.)) >= 2015) %>% subset(year(time(.)) <= 2020) %>% ### filter to years of interest
